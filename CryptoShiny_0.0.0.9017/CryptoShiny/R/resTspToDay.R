...
resTspToDay <- function(resTsp, CrpSymbols) {
  max <-
    floor_date(as.POSIXct(max(resTsp$time), origin = "1970-01-01"), unit = "day") + 3600 *
    24
  min <-
    floor_date(as.POSIXct(min(resTsp$time), origin = "1970-01-01"), unit = "day")
