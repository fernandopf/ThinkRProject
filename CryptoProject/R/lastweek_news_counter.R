...
lastweek_news_counter <- function(cryptocurrency = "BTC" ){
  ...
  oneweek <- round(as.numeric(Sys.time()-as.difftime(7, units="days")))

  data <- Tsp_data(now) #called the function Tsp_data inside this Package
  time <- min(data$time)
  timelimit <- oneweek
  ...
  count <- data %>%
    mutate(mentioned = as.numeric(grepl(pattern = keywords, body, ignore.case = TRUE))) %>%
