...
#'
#' @examples
crypto <- function(timeframe, firstDay, lastDay, crypocurrency = "BTC", comparison = "USD", n_MA, n_quick_MACD, n_slow_MACD, n_signal_MACD){
  if (timeframe %in% c("Week", "week", "Month", "month")){
    df <- day_hour(timeframe = "day", firstDay, lastDay, crypocurrency, comparison);
    ...
    df <- day_hour(timeframe, firstDay, lastDay, crypocurrency, comparison);
